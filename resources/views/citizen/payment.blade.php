                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span
                                class="font-semibold">{{ optional($invoice->application->certificateType)->name ?? 'সনদ সেবা' }}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span
                                class="font-semibold {{ $invoice->payment_status === 'paid' ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold">৳
                                {{ number_format($invoice->amount - ($invoice->vat_amount + $invoice->service_charge), 2) }}</span>
                        </div>
            ...
            <div id="paymentForm">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- bKash Option -->
                        <div class="relative">
                            <input type="radio" name="gateway" id="gateway_bkash" value="bkash" class="hidden peer"
                                data-form-id="bkash_form" checked>
                            <label for="gateway_bkash"
                                ...
                                <svg class="w-6 h-6 text-green-500 ml-auto hidden peer-checked:block"
...
<script>
    document.getElementById('bkashSubmitBtn').addEventListener('click', function() {
        fetch("{{ route('citizen.payments.grantToken', $invoice->id) }}")
            .then(response => response.json())
            .then(data => {
                if (data.token) {
                    // Proceed with bKash payment using the token
                    alert('bKash Token: ' + data.token);
                    // Here you would typically call the bKash payment SDK with the token
                } else if (data.error) {
